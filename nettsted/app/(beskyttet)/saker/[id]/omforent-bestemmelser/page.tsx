...
export default async function OmforentBestemmelser({
  ...
  const bestemmelser = (data ?? []) as OmforentRad[];
    ...
    <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-10">
      ...
      <section className="mt-8 rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-4 text-sm leading-6 text-slate-600">
              Her samles de omforente bestemmelsene som skal inngå i samlet
        ...
        <div className="mt-6 space-y-4">
          {bestemmelser.length > 0 ? (
            bestemmelser.map((rad) => {
              const kapittelInfo =
            ...
            <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-5">
              <p className="text-sm font-medium text-slate-600">
                Ingen omforente bestemmelser er lagret ennå.
              </p>
            </div>
          )}
        </div>
